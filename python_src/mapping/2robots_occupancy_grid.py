...
def visualize(G, robotX1, robotY1, robotX2, robotY2):
    plt.imshow(G.T, cmap=plt.cm.Greys, origin='lower', extent=[LOWEST_X, DIM_X+LOWEST_X, LOWEST_Y, DIM_Y+LOWEST_Y])
    plt.grid()
    plt.plot(robotX1[:i], robotY1[:i], label='robot1 path')
    plt.plot(robotX2[:i], robotY2[:i], label='robot2 path')
    plt.xlabel('X, m')
    plt.ylabel('Y, m')
    plt.legend(loc=4)
...
def preprocessing(poses, scans):
    ...
    for i in range(poses.shape[0]):
        scan_x = np.array( [scans[i,0], scans[i,3], scans[i,6], scans[i,9]] )
        scan_y = np.array( [scans[i,1], scans[i,4], scans[i,7], scans[i,10]] )
        # remove nans:
...
robotX1 = poses1[:,0]; robotX2 = poses2[:,0];
robotY1 = poses1[:,1]; robotY2 = poses2[:,1]; 
robotZ1 = poses1[:,2]; robotZ1 = poses2[:,2]; 
...
for i in range(len(poses)):
    ...
    grid = convert2map( pose[:2], scans[i] - np.array([LOWEST_X,LOWEST_Y]), map_pix=RESOLUTION, map_size=map_size, prob=0.02 )
